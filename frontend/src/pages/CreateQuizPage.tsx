...
import {
    ...
    ChevronDown, ChevronRight, ArrowUp, ArrowDown, Eye, Shield, Shuffle, ArrowLeft,
...
const CreateQuizPage: React.FC = () => {
    ...
    // Auto-create draft on first load if not in edit mode
    useEffect(() => {
        const autoCreateDraft = async () => {
            if (!isEditMode && !hasCreatedDraft.current) {
                ...
                try {
                    const quizData = {
                        ...
                        questions: [], // Send empty array to avoid initial validation errors
                        totalMarks: 0,
                        passingMarks: 0,
                        isPublished: false
                    };
                    const createdQuiz = await quizService.createQuiz(quizData);
                    const quizId = createdQuiz.id || createdQuiz._id;
                    
                    setQuizInfo(prev => ({ ...prev, title: createdQuiz.title || 'Untitled Quiz' }));
                    navigate(`/create-quiz?edit=${quizId}`, { replace: true });
                } catch (error) {
            ...
            <div className="max-w-[1100px] mx-auto">
                ...
                <header className="flex justify-between items-start pt-2 pb-0 sm:pb-4 border-b border-transparent">
                    <div className="flex items-center gap-3 sm:gap-4">
                        <button onClick={() => navigate('/my-quizzes')} className="p-2 sm:p-2.5 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 shadow-sm transition-all" title="Back to My Quizzes">
                            <ArrowLeft size={20} className="sm:w-6 sm:h-6" />
                        </button>
                        <div>
                            <h1 className="text-[1.5rem] sm:text-[2rem] font-extrabold text-slate-800 mb-0.5 sm:mb-1 leading-tight">
                                {isEditMode && quizInfo.status === 'published' ? 'Edit Quiz' : 'Create Quiz'}
                            </h1>
