...
const ScoreDistributionChart: React.FC<ScoreDistributionChartProps> = ({ data }) => {
    return (
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-[300px]">
            <h3 className="text-sm font-bold text-slate-700 mb-4 px-2">1. Score Distribution</h3>
            <div className="flex-1 min-h-0">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        />
                        <Bar dataKey="count" fill="#6366f1" radius={[4, 4, 0, 0]} name="Students" animationDuration={1000} />
                    </BarChart>
                </ResponsiveContainer>
            </div>
        </div>
